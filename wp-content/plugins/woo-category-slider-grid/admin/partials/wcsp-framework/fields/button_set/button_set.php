		...
		public function render() {
			...
			if ( ! empty( $args['options'] ) ) {
				...
				foreach ( $args['options'] as $key => $option ) {
					...
					echo '<input type="' . esc_attr( $type ) . '"  ' . esc_attr( $pro_only ) . ' name="' . esc_attr( $this->field_name( $extra ) ) . '" value="' . esc_attr( $key ) . '"' . wp_kses_post( $this->field_attributes() . $checked ) . '/>' . wp_kses_post( $pro_only_text ) . '';
