...
if ( ! class_exists( 'Merchant_Admin_Modules' ) ) {
	class Merchant_Admin_Modules {
				...
				if ( isset( self::modules_translation_data()[ $module_id ] ) ) {
					$modules[ $module_data['section'] ]['modules'][ $module_id ] = array_merge(
						...
						array(
							...
							'ai_examples'  => self::get_module_ai_examples( $module_id ),
						)
					);
				}
			}
		...
		/**
		 * Get a module's AI prompt examples.
		 *
		 * Prefers the module's own class (see Merchant_Add_Module::get_ai_examples())
		 * and falls back to modules_translation_data() for modules that don't yet
		 * define their own.
		 *
		 * @param string $module_id The module identifier.
		 *
		 * @return array Array with 'blurb' and 'prompts' keys, or an empty array.
		 */
		private static function get_module_ai_examples( $module ) {
			$module_instance = Merchant_Modules::get_module( $module );

			if ( $module_instance && ! empty( $module_instance->get_ai_examples() ) ) {
				return $module_instance->get_ai_examples();
			}

			return self::modules_translation_data()[ $module ]['ai_examples'] ?? array();
		}
			...
			if ( isset( self::$modules_data[ $module ] ) && isset( self::modules_translation_data()[ $module ] ) ) {
				return array_merge(
					...
					array(
						...
						'ai_examples'  => self::get_module_ai_examples( $module ),
					)
				);
			}
