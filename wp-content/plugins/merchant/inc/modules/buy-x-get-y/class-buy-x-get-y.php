...
class Merchant_Buy_X_Get_Y extends Merchant_Add_Module {
	...
	public function __construct() {
		...
		$this->ai_examples = array(
			'blurb'   => esc_html__( 'See what AI can do for your Buy X, Get Y campaigns, from launching a new deal to adjusting one already running.', 'merchant' ),
			'prompts' => array(
				esc_html__( 'Explore the abilities WPVibe gives you access to, then create a Buy 2 Get 1 Free campaign for the Summer Tees category that runs through the end of the month', 'merchant' ),
				esc_html__( 'Check what abilities you have available through WPVibe, then set up a Buy 1 Get 1 at 50% off deal for our Winter Jackets collection, capped at 3 free items per order', 'merchant' ),
				esc_html__( "Look at your available WPVibe abilities, then update the button text on the Summer Tees Buy X Get Y campaign to 'Add My Free Item'", 'merchant' ),
				esc_html__( 'See what abilities WPVibe exposes, then limit the Buy 3 Get 1 Free campaign to only the Clearance category', 'merchant' ),
			),
		);
