...
class Merchant_Stock_Scarcity extends Merchant_Add_Module {
	...
	public function __construct() {
		...
		$this->ai_examples = array(
			'blurb'   => esc_html__( 'See what AI can do for your low-stock urgency box, from setting the stock threshold that triggers it to rewording its message and restyling how it looks.', 'merchant' ),
			'prompts' => array(
				esc_html__( 'Explore the abilities WPVibe gives you access to, then show the low-stock urgency box only once a product drops below 5 units in stock', 'merchant' ),
				esc_html__( "Check what abilities you have available through WPVibe, then reword the low-stock message to 'Almost gone, only {stock} left in stock'", 'merchant' ),
				esc_html__( 'Look at your available WPVibe abilities, then show the urgency box on shop and category archive pages too, not just the single product page', 'merchant' ),
				esc_html__( 'See what abilities WPVibe exposes, then restyle the low-stock urgency box\'s progress bar with a green-to-red gradient and bump the message font size up to 18px', 'merchant' ),
			),
		);
