...
class Merchant_Trust_Badges extends Merchant_Add_Module {
	...
	public function __construct() {
		...
		$this->ai_examples = array(
			'blurb'   => esc_html__( 'See what AI can do for your trust badges, from choosing which ones appear on your product pages to restyling the heading and how the badges line up.', 'merchant' ),
			'prompts' => array(
				esc_html__( 'Explore the abilities WPVibe gives you access to, then show all three built-in trust badges on my product pages', 'merchant' ),
				esc_html__( "Check what abilities you have available through WPVibe, then change the text above the trust badges from 'Product Quality Guaranteed!' to 'Shop With Confidence'", 'merchant' ),
				esc_html__( 'Look at your available WPVibe abilities, then left-align the trust badges and set the heading font size to 18px', 'merchant' ),
				esc_html__( 'See what abilities WPVibe exposes, then cap the trust badge images at 60px wide and 60px tall', 'merchant' ),
			),
		);
		...
		/**
		 * Add trust badges inside the Quick View modal.
		 *
		 * @since 2.3.0
		 */
		add_action( 'merchant_quick_view_product_summary', array( $this, 'trust_badges_output' ), 30 );
	...
	public function trust_badges_output() {
		...
		// `is_product()` is false during the Quick View AJAX request, so also allow
		// output when we're rendering inside the Quick View modal.
		if ( ! is_product() && ! did_action( 'merchant_quick_view_before_add_to_cart' ) ) {
			return;
		}
