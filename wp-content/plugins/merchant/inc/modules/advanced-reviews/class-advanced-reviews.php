	...
	class Merchant_Advanced_Reviews extends Merchant_Add_Module {
		...
		public function __construct( Merchant_Product_Dummy $dummy_product ) {
			...
			$this->ai_examples = array(
				'blurb'   => esc_html__( 'See what AI can do for your product reviews, from turning on photo uploads and a review images carousel to rewarding shoppers with a discount for leaving one.', 'merchant' ),
				'prompts' => array(
					esc_html__( 'Explore the abilities WPVibe gives you access to, then turn on photo reviews and let shoppers upload up to 4 photos with each review', 'merchant' ),
					esc_html__( 'Check what abilities you have available through WPVibe, then enable the review images carousel and show 4 images per slide', 'merchant' ),
					esc_html__( 'Look at your available WPVibe abilities, then offer a 10% discount for leaving a review and have the code expire after 30 days', 'merchant' ),
					esc_html__( 'See what abilities WPVibe exposes, then send an automated review-request email 5 days after an order is completed', 'merchant' ),
				),
			);
