...
class Merchant_Cart_Reserved_Timer extends Merchant_Add_Module {
	...
	public function __construct() {
		...
		$this->ai_examples = array(
			'blurb'   => esc_html__( 'See what AI can do for your cart reservation timer, from setting how long items stay reserved to choosing whether the cart clears when time runs out.', 'merchant' ),
			'prompts' => array(
				esc_html__( 'Explore the abilities WPVibe gives you access to, then set the cart reservation timer to reserve items for 15 minutes', 'merchant' ),
				esc_html__( 'Check what abilities you have available through WPVibe, then have the cart reservation timer clear the cart when it expires instead of just hiding it', 'merchant' ),
				esc_html__( "Look at your available WPVibe abilities, then change the reserved-cart message to 'Only a few left — your items are held for a limited time'", 'merchant' ),
				esc_html__( 'See what abilities WPVibe exposes, then switch the cart reservation timer\'s icon to the hourglass and set its background color to a soft red', 'merchant' ),
			),
		);
