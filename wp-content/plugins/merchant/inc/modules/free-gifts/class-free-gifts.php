...
class Merchant_Free_Gifts extends Merchant_Add_Module {
	...
	public function __construct() {
		...
		$this->ai_examples = array(
			'blurb'   => esc_html__( 'See what AI can do for your free-gift offers, from rewarding a spending goal to tying a gift to a coupon code.', 'merchant' ),
			'prompts' => array(
				esc_html__( 'Explore the abilities WPVibe gives you access to, then create a free-gift offer that gives away a Travel Mug when a customer spends over $100', 'merchant' ),
				esc_html__( 'Check what abilities you have available through WPVibe, then set up a coupon-based free-gift offer that adds a Travel Mug once a shopper applies the WELCOME10 coupon', 'merchant' ),
				esc_html__( "Look at your available WPVibe abilities, then update the message shown when shoppers hit the goal on the Travel Mug free-gift offer to 'Congrats, your free Travel Mug is unlocked!'", 'merchant' ),
				esc_html__( 'See what abilities WPVibe exposes, then limit the Travel Mug free-gift offer so the gift is only claimable on purchases from the Drinkware category', 'merchant' ),
			),
		);
