...
class Merchant_Login_Popup extends Merchant_Add_Module {
	...
	public function __construct() {
		...
		$this->ai_examples = array(
			'blurb'   => esc_html__( 'See what AI can do for your login popup, from wording the login link and welcome message to resizing and restyling the popup itself.', 'merchant' ),
			'prompts' => array(
				esc_html__( "Explore the abilities WPVibe gives you access to, then change the login popup link text to 'Sign In or Register'", 'merchant' ),
				esc_html__( 'Check what abilities you have available through WPVibe, then turn off the login popup welcome message shown to logged-in users', 'merchant' ),
				esc_html__( "Look at your available WPVibe abilities, then set the login popup's welcome message to 'Hi {display_name}, welcome back!'", 'merchant' ),
				esc_html__( "See what abilities WPVibe exposes, then set the login popup's width to 500px and change its background color to white", 'merchant' ),
			),
		);
