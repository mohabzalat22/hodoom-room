...
class Merchant_Product_Audio extends Merchant_Add_Module {
	...
	public function __construct() {
		...
		$this->ai_examples = array(
			'blurb'   => esc_html__( 'See what AI can do for your product audio, from setting a featured track on a product to building an audio gallery or switching on autoplay.', 'merchant' ),
			'prompts' => array(
				esc_html__( 'Explore the abilities WPVibe gives you access to, then set the featured audio on the Midnight Jazz Vinyl product so it plays in place of the featured image on the shop and archive pages', 'merchant' ),
				esc_html__( 'Check what abilities you have available through WPVibe, then add an audio gallery to the Midnight Jazz Vinyl product with a few preview tracks and position it before the image gallery', 'merchant' ),
				esc_html__( 'Look at your available WPVibe abilities, then turn on autoplay so the audio player starts on its own for shoppers', 'merchant' ),
				esc_html__( 'See what abilities WPVibe exposes, then enable the shortcode option so I can place the product audio player manually with [merchant_module_product_audio]', 'merchant' ),
			),
		);
