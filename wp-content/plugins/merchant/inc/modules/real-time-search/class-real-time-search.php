...
class Merchant_Real_Time_Search extends Merchant_Add_Module {
	...
	public function __construct() {
		...
		$this->ai_examples = array(
			'blurb'   => esc_html__( 'See what AI can do for your real-time product search, from setting how many results appear and how they are sorted to letting shoppers search by SKU.', 'merchant' ),
			'prompts' => array(
				esc_html__( 'Explore the abilities WPVibe gives you access to, then show up to 8 products in the real-time search results', 'merchant' ),
				esc_html__( 'Check what abilities you have available through WPVibe, then sort the search results by price from highest to lowest', 'merchant' ),
				esc_html__( 'Look at your available WPVibe abilities, then let shoppers search by SKU as well as product name', 'merchant' ),
				esc_html__( 'See what abilities WPVibe exposes, then also list matching product categories in the results and widen the results box to 600px', 'merchant' ),
			),
		);
