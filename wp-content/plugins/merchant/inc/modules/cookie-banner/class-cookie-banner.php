...
class Merchant_Cookie_Banner extends Merchant_Add_Module {
	...
	public function __construct() {
		...
		$this->ai_examples = array(
			'blurb'   => esc_html__( 'See what AI can do for your cookie banner, from rewording its message to switching its placement and linking it to your privacy policy.', 'merchant' ),
			'prompts' => array(
				esc_html__( 'Explore the abilities WPVibe gives you access to, then reword the cookie banner message to explain that we only use cookies to improve the shopping experience', 'merchant' ),
				esc_html__( 'Check what abilities you have available through WPVibe, then move the cookie banner to the fixed bottom position instead of floating', 'merchant' ),
				esc_html__( "Look at your available WPVibe abilities, then point the cookie banner's 'Learn More' link at our privacy policy page", 'merchant' ),
				esc_html__( "See what abilities WPVibe exposes, then change the cookie banner's accept button to say 'Got it' and remember the choice for 30 days", 'merchant' ),
			),
		);
