  ...
  merchant.modules.quickView = {
    init: function init() {
      ...
      $modals.each(function () {
        ...
        var $navPrev = $modal.find('.merchant-quick-view-nav-prev');
        var $navNext = $modal.find('.merchant-quick-view-nav-next');
        ...
        var isOpen = false;
        var currentIndex = -1;
        var openedViaOwnPushState = false;
        function updateNavState(productId) {
          var productIds = self.getProductIds();
          currentIndex = productIds.indexOf(productId);
          if (productIds.length < 2 || -1 === currentIndex) {
            $navPrev.removeClass('merchant-quick-view-nav-visible');
            $navNext.removeClass('merchant-quick-view-nav-visible');
            return;
          }
          $navPrev.toggleClass('merchant-quick-view-nav-visible', currentIndex > 0);
          $navNext.toggleClass('merchant-quick-view-nav-visible', currentIndex < productIds.length - 1);
        }
        function loadProduct(productId, historyAction) {
          $content.empty();
          $modal.addClass('merchant-show');
          $modal.addClass('merchant-loading');
          isOpen = true;
          if (window.merchant.setting.sync_url_hash) {
            if ('push' === historyAction) {
              history.pushState({
                merchantQuickView: productId
              }, '', '#merchant-quick-view-' + productId);
        ...
        $(document).on('click', '.merchant-quick-view-button', function (e) {
          e.preventDefault();
          openedViaOwnPushState = !!window.merchant.setting.sync_url_hash;
          loadProduct($(this).data('product-id'), 'push');
        });
        $navPrev.on('click', function (e) {
          e.preventDefault();
          var productIds = self.getProductIds();
          if (currentIndex > 0) {
            loadProduct(productIds[currentIndex - 1], 'replace');
          }
        });
        ...
        $closeButton.on('click', function (e) {
          ...
          if (window.merchant.setting.sync_url_hash) {
            if (openedViaOwnPushState) {
              history.back();
            } else {
              history.replaceState(null, '', location.pathname + location.search);
            }
            openedViaOwnPushState = false;
          }
        });
        if (window.merchant.setting.sync_url_hash) {
          window.addEventListener('popstate', function () {
            var match = location.hash.match(/^#merchant-quick-view-(\d+)$/);
            if (match) {
              loadProduct(parseInt(match[1], 10), 'none');
            } else if (isOpen) {
              isOpen = false;
              $modal.removeClass('merchant-show');
              $inner.removeClass('merchant-show');
          ...
          var initialMatch = location.hash.match(/^#merchant-quick-view-(\d+)$/);
          if (initialMatch) {
            loadProduct(parseInt(initialMatch[1], 10), 'none');
          }
        }
      });
    },
    /**
     * Collect product IDs for every Quick View button currently in the DOM, in document order.
     *
     * @return {Array} Ordered, de-duplicated product IDs.
     */
    getProductIds: function getProductIds() {
      var ids = [];
      $('.merchant-quick-view-button[data-product-id]').each(function () {
